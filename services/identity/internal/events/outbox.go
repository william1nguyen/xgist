// Package events relays identity's transactional outbox to Kafka,
// consumes cross-service deletion-completion events, and reconciles
// overdue pending deletions.
package events
...
// DeletionRequestedTopic is the topic identity publishes to when an
// account deletion is requested, per ADR 0003's mn.<domain>.<event>.v1
// naming and identity.md's deletion flow.
const DeletionRequestedTopic = "mn.identity.account.deletion.requested.v1"
...
// Publisher. It commits a record as published only after Publish
// succeeds, so a crash mid-publish is safe: the record is retried on the
// next tick.
type Relay struct {
