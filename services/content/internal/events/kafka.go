...
// RunConsumer reads deletion-requested events until ctx is canceled,
// applying each through consumer. An offset commits only after Handle
// succeeds, so a crash mid-processing redelivers rather than loses the
// event.
func RunConsumer(ctx context.Context, brokers []string, consumer *Consumer) error {
	reader := kafka.NewReader(kafka.ReaderConfig{
		Brokers:     brokers,
		GroupTopics: []string{DeletionRequestedTopic},
