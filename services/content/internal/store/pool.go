// Package store implements content's PostgreSQL repositories: the
// content, deletion, outbox, and inbox persistence boundaries defined by
// internal/content, internal/deletion, and internal/events.
package store
...
// content does not migrate itself at boot: migrations run externally
// through `make content:migrate` (see migrations/), the same way a
...
func CheckMigrated(ctx context.Context, pool *pgxpool.Pool) error {
	...
	if !exists {
		return fmt.Errorf("no flyway_schema_history table found; run `make content:migrate` before starting the service")
	}
	return nil
}
