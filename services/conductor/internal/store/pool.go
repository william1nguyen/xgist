// Package store implements conductor's PostgreSQL repositories: the
// workflow, outbox, inbox, and DLQ persistence boundaries defined by
// internal/workflow and internal/events.
package store
...
// conductor does not migrate itself at boot: migrations run externally
// through `make conductor:migrate` (see migrations/), the same way a
...
func CheckMigrated(ctx context.Context, pool *pgxpool.Pool) error {
	...
	if !exists {
		return fmt.Errorf("no flyway_schema_history table found; run `make conductor:migrate` before starting the service")
	}
	return nil
}
