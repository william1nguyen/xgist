...
export function AudioSummaryPanel({ audios }: { audios: SummaryAudio[] }) {
	...
	if (audios.length === 0) {
		return (
			<p className="text-muted-foreground text-sm">{t("audioPanel.noAudio")}</p>
		);
	}
			...
			{audios.map((audio) => (
				...
				>
					<div className="flex items-center gap-3">
						...
						<div className="min-w-0 flex-1">
							<p className="font-medium text-sm">
								{t("audioPanel.voiceLabel", { voice: audio.voice })}
							</p>
					...
					{audio.url ? (
						...
						<p className="text-muted-foreground text-xs">
							{audio.status === "READY"
								? t("audioPanel.playbackUnavailable")
								: t("audioPanel.generationFailed")}
						</p>
					)}
				</div>
			))}
