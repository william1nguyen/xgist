...
// RunConsumer reads workflow-status and account-deletion events from
// their topics until ctx is canceled, applying each through consumer. An
// offset commits only after Handle succeeds, so a crash mid-processing
// redelivers rather than loses the event.
func RunConsumer(ctx context.Context, brokers []string, consumer *Consumer) error {
	reader := kafka.NewReader(kafka.ReaderConfig{
		Brokers:     brokers,
		GroupTopics: []string{StatusChangedTopic, AccountDeletionRequestedTopic},
		GroupID:     eventsConsumerGroup,
	})
	defer reader.Close()
