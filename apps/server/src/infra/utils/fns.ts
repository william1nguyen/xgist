...
export const trimAndNormalize = (value: string, isLower = false) => {
  ...
  return isLower ? v.replace(/\s+/g, " ").toLowerCase() : v.replace(/\s+/g, " ");
};
...
export const daysBetween = (start: Date, end: Date, timezone: TimeZone = "UTC"): number => {
...
export const isSubArray = (subArray: string[], mainArray: string[]): boolean => {
...
export const maskContent = (
  ...
  const { unmaskedEndCharacters = 3, unmaskedStartCharacters = 3 } = options || {};
