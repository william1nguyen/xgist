...
export default function CreditSummary({ totalCost, balance }: CreditSummaryProps) {
  ...
  return (
    <div className="flex items-center justify-between rounded-xl border border-border bg-card px-4 py-3">
      <div className="flex items-center gap-2">
        ...
        <span className={`text-sm font-semibold tabular-nums transition-colors ${costColor}`}>
          {animatedCost} credits
        </span>
      </div>
      <div className="flex items-center gap-2">
        <span className="text-xs text-muted-foreground">Balance: {balance}c</span>
        {insufficient && (
          <NavLink
            to="/billing"
