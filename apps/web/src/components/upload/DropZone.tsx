...
export default function DropZone({ onFile, onError, error }: DropZoneProps) {
  ...
  const validate = (file: File): string | null => {
    if (!ALLOWED_TYPES.has(file.type)) return "Unsupported format. Use MP4, MOV, MKV, WebM, MP3, WAV, or M4A."
  ...
  return (
    <div className="space-y-2">
      ...
      >
        ...
        <p className="text-xs text-muted-foreground">MP4 · MOV · MKV · WebM · MP3 · WAV · M4A · max 500MB</p>
        <input
          ref={inputRef}
          type="file"
